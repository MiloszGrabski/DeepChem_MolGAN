...
class GraphConvolutionLayer(layers.Layer):
    ...
    def __init__(
        self, units, activation=None, dropout_rate=0.0, edges=5, name="", **kwargs
    ):
    ...
    def call(self, inputs, training=False):
        ...
        if ic < 2:
            raise ValueError(
                "GraphConvolutionLayer requires at leat two inputs: [adjacency_tensor, node_features_tensor]"
            )
...
class GraphAggregationLayer(layers.Layer):
    ...
    def __init__(self, units, activation=None, dropout_rate=0.0, name="", **kwargs):
...
class MultiGraphConvolutionLayer(layers.Layer):
    ...
    def __init__(
        ...
        if len(units) < 2:
            raise ValueError(
                "Single layer unit provided, this layer is for multiple convolutions only. Use GraphConvolutionLayer instead."
            )
...
class GraphEncoderLayer(layers.Layer):
    ...
    def __init__(
        self, units, activation="tanh", dropout_rate=0.0, edges=5, name="", **kwargs
    ):
        ...
        self.multi_graph_convolution_layer = MultiGraphConvolutionLayer(
            self.graph_convolution_units, self.activation, self.dropout_rate, self.edges
        )
